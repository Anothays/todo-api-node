...
async function ghRaw(owner, repo, path) {
  const res = await fetch(`https://raw.githubusercontent.com/${owner}/${repo}/main/${path}`, { headers });
  if (!res.ok) return null;
  return res.text();
}
    ...
    if (lang === "python") {
      if (trimmed.startsWith("#")) {
        ...
        if (inBlock) {
          comments++;
          inBlock = false;
        } else if (trimmed.indexOf(delim, 3) !== -1) {
          comments++;
        } else {
          comments++;
          inBlock = true;
...
const CHECKS = {
  ...
  lint_pass: {
    ...
    run: async (owner, repo, _team, ctx) => {
      const result = findGreenStep(ctx, [
        "lint",
        "linter",
        "format",
        "eslint",
  ...
  tests_exist: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      if (realTests === 0)
        return { pass: false, detail: `${testFiles.length} test file(s) but no real assertions/imports` };
  ...
  tests_pass: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      if (ctx.lastRun.conclusion !== "success")
        return { pass: false, detail: `Pipeline not green (${ctx.lastRun.name} #${ctx.lastRun.run_number})` };
  ...
  coverage_70: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      const covStepResult = findGreenStep(ctx, [
        "coverage",
        "cov",
        "codecov",
        "coveralls",
  ...
  deployed: {
    ...
    run: async (_owner, _repo, team) => {
      ...
      try {
        ...
        try {
          const todosRes = await fetch(team.prod_url.replace(/\/+$/, "") + "/todos", {
    ...
    run: async (owner, repo, team, ctx) => {
      ...
      if (hasGHEnvs) {
        if (!team.staging_url) {
          return {
            pass: false,
            detail: `GitHub environments: ${envDetail} — but no staging_url in teams.json (add it via PR!)`,
          };
        }
      ...
      if (ctx.lastRunJobs) {
        ...
        for (const job of ctx.lastRunJobs) {
          ...
          if (
            !hasStaging &&
            (jn.includes("staging") ||
              jn.includes("deploy-staging") ||
              jn.includes("deploy_staging") ||
              jn.includes("dev"))
          ) {
            hasStaging = true;
            stagingJob = job.name;
          ...
          if (
            !hasProd &&
            (jn.includes("production") ||
              jn.includes("deploy-prod") ||
              jn.includes("deploy_prod") ||
              jn.includes("deploy-production"))
          ) {
            hasProd = true;
            prodJob = job.name;
          }
        ...
        if (hasStaging && hasProd) {
          return {
            pass: false,
            detail: `Multi-env jobs: "${stagingJob}" + "${prodJob}" — but no staging_url in teams.json (add it via PR!)`,
          };
        }
      }
  ...
  dependabot: {
    ...
    run: async (owner, repo, _team, ctx) => {
      const depbot =
        (await ghRaw(owner, repo, ".github/dependabot.yml")) || (await ghRaw(owner, repo, ".github/dependabot.yaml"));
      if (depbot && depbot.includes("package-ecosystem")) {
        return { pass: true, detail: "dependabot config with valid setup" };
      }
      const renovate =
        (await ghRaw(owner, repo, "renovate.json")) ||
        (await ghRaw(owner, repo, ".github/renovate.json")) ||
        (await ghRaw(owner, repo, ".github/renovate.json5"));
...
const EXPERT_CHECKS = {
  ci_notifications: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      for (const wf of ctx.workflows) {
        ...
        if (
          lower.includes("discord-webhook") ||
          lower.includes("slack-webhook") ||
          lower.includes("slackapi/") ||
          lower.includes("discord_webhook") ||
          lower.includes("8398a7/action-slack") ||
          lower.includes("rtcamp/action-slack") ||
  ...
  reusable_workflows: {
    ...
    run: async (owner, repo, _team, ctx) => {
      for (const wf of ctx.workflows) {
        ...
        if (
          /uses:\s*['"]?\.\/\.github\/workflows\//m.test(content) ||
  ...
  smoke_tests: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      for (const wf of ctx.workflows) {
        const content = wf.content;
        if (
          /smoke[-_]?test/i.test(content) ||
          /post[-_]?deploy[-_]?test/i.test(content) ||
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      if (readme) {
        ...
        for (const m of monitors) {
          if (lower.includes(m)) {
            return {
              pass: true,
              detail: `${m} reference in README (no verifiable URL — add a badge/status page link!)`,
            };
          }
        }
      }
      ...
      for (const wf of ctx.workflows) {
        ...
        if (
          ...
          return {
            pass: true,
            detail: `Monitoring integration in ${wf.path} (add a status badge in README for full credit)`,
          };
        }
  ...
  auto_recovery: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      const healthResult = findGreenStep(ctx, [
        "health-check",
        "health_check",
        "healthcheck",
        "smoke",
      ...
      return {
        pass: false,
        detail: "No auto-recovery mechanisms found (add workflow_dispatch, health checks, or recovery workflows)",
      };
    },
  },
  ci_cache: {
  ...
  contract_testing: {
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      if (pkg) {
        ...
        if (
          lower.includes("swagger-jsdoc") ||
          lower.includes("swagger-autogen") ||
          lower.includes("tsoa") ||
          lower.includes("nestjs/swagger")
        ) {
          hasGenerator = true;
        }
      }
      ...
      if (!hasValidator) {
        for (const wf of ctx.workflows) {
          const lower = wf.content.toLowerCase();
          if (
            lower.includes("dredd") ||
            lower.includes("schemathesis") ||
            lower.includes("prism") ||
            lower.includes("spectral") ||
            lower.includes("openapi-validate")
          ) {
            hasValidator = true;
            break;
          }
      ...
      if (!hasValidator && pkg) {
        ...
        if (
          lower.includes("dredd") ||
          lower.includes("schemathesis") ||
          lower.includes("@stoplight/prism") ||
          lower.includes("@stoplight/spectral")
        ) {
          hasValidator = true;
        }
      }
      ...
      if (hasSpec || hasGenerator) {
        return {
          pass: false,
          detail: `API spec found (${specFile || "auto-generated"}) but no contract validation in CI`,
        };
      }
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      if (ctx.workflows) {
        for (const wf of ctx.workflows) {
          const lower = wf.content.toLowerCase();
          if (
            lower.includes("release-please") ||
            lower.includes("semantic-release") ||
            lower.includes("conventional-changelog") ||
            lower.includes("auto-changelog") ||
            lower.includes("standard-version")
          ) {
            const tool = [
              "release-please",
              "semantic-release",
    ...
    run: async (owner, repo, _team, ctx) => {
      ...
      for (const wf of ctx.workflows) {
        ...
        if (
          ...
          lower.includes("blue_green") ||
          lower.includes("bluegreen") ||
          lower.includes("canary") ||
          lower.includes("rolling-update") ||
          lower.includes("rolling_update")
        ) {
          const strategy = ["blue-green", "blue_green", "bluegreen", "canary", "rolling-update", "rolling_update"].find(
            (s) => lower.includes(s),
          );
      ...
      if (ctx.readme) {
        ...
        if (
          lower.includes("blue/green") ||
          lower.includes("blue-green") ||
          lower.includes("canary deployment") ||
          lower.includes("rolling update")
        ) {
...
async function scoreTeam(team) {
  ...
  if (lastRun) {
    ...
    console.log("lastRunJobs --> ", lastRunJobs, lastRunJobs.steps);
  }
  ...
  return {
    ...
    repo: team.repo,
    prod_url: team.prod_url,
    staging_url: team.staging_url,
    total,
    maxTotal,
    expert,
    maxExpert,
